...
public class EasyTouch : MonoBehaviour {
	...
	// The unit used for the stationary tolerance
	public enum ToleranceUnit { Millimeters, Pixels }
	public ToleranceUnit toleranceUnit = ToleranceUnit.Pixels;
	// The stationary tolerance, in the chosen unit
	public float _StationaryTolerance; 
	//Returns the stationary tolerance in pixels (as used in EasyTouch's code)
	public float StationaryTolerance
	{
		get
		{
			if (toleranceUnit == ToleranceUnit.Millimeters)
				return TTDUtils.MILLIMETERS_TO_PIXELS * _StationaryTolerance;
			else
				return _StationaryTolerance;
		}
	}
	...
	public static void SetStationaryTolerance(float tolerance){
		if (EasyTouch.instance)
			EasyTouch.instance._StationaryTolerance = tolerance;
	}
